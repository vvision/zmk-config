...
- Add escape on shift thumb key.
- Add comma, dot, minus and slash keys on the number layer to reflect ergol `navigation` layer.

### Changed

- Swap position of ^ and @ in the `symbol` layer.
- Move tab one key higher in `navigation` layer.
- Replace old tab key with sticky shift in `navigation` layer.

### Removed

- Remove tab key on thumb key in `navigation` layer.
- Remove backspace key in `navigation` layer.
